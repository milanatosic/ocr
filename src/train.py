...
def evaluate(model, loader, ctc_loss):
    """
    Evaluacija modela na zadatom loaderu.
    collate_fn vraća: imgs, labels_concat, label_lengths, input_lengths, label_strs
    """
    model.eval()
    total_loss = 0.0
    total_cer = 0.0
    ...
    with torch.no_grad():
        for imgs, labels_concat, label_lengths, input_lengths, label_strs in loader:
            imgs          = imgs.to(device)
            ...
            # input_lengths ne sme biti veći od T
...
def train():
    ...
    for epoch in range(1, CONFIG["num_epochs"] + 1):
        ...
        for imgs, labels_concat, label_lengths, input_lengths, _ in tqdm(train_loader, desc=f"Epoha {epoch}"):
            imgs          = imgs.to(device)
            labels_concat = labels_concat.to(device)
            label_lengths = label_lengths.to(device)
            input_lengths = input_lengths.to(device)
        ...
        if val_loss < best_val_loss:
            ...
            if patience_counter >= CONFIG["patience"]:
                print(f"Early stopping aktiviran na epohi {epoch}.")
                break
